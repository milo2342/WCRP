...
function commandData() {
  ...
  add(new SlashCommandBuilder().setName('ticket-panel').setDescription('Create and manage ticket panels.').setDefaultMemberPermissions(PermissionFlagsBits.ManageGuild).setDMPermission(false)
    .addSubcommand(sc => sc.setName('create').setDescription('Create a ticket panel.')
      ...
      .addStringOption(o => o.setName('embed-color').setDescription('Embed colour, e.g. #7C3AED or 7C3AED'))
    ...
    .addSubcommand(sc => sc.setName('edit').setDescription('Edit a ticket panel appearance and text.')
      ...
      .addStringOption(o => o.setName('embed-color').setDescription('Embed colour, e.g. #7C3AED'))
  ...
  switch (interaction.commandName) {
    ...
    case 'ticket-panel': {
      ...
      if (sub === 'edit') {
        ...
        if (embedColor !== null) panel.color = parseHexColor(embedColor, panel.color ?? COLORS.primary);
        ...
        if (oldMessage) {
          const button = new ButtonBuilder().setCustomId(`panel_open:${panel.id}`).setLabel((panel.buttonLabel || 'Open Ticket').slice(0, 80)).setStyle(buttonStyleFromName(panel.buttonStyle));
          if (panel.buttonEmoji) button.setEmoji(panel.buttonEmoji);
          await oldMessage.edit({ embeds: [ticketPanelEmbed(panel)], components: [new ActionRowBuilder().addComponents(button)] }).catch(() => null);
        }
      ...
      if (sub === 'hub') {
        ...
        const hubColor = parseHexColor(interaction.options.getString('embed-color'), panels[0]?.color ?? COLORS.primary);
        const embed = new EmbedBuilder()
          .setColor(hubColor)
          .setTitle(title)
          .setDescription(description)
          .addFields(
            { name: 'Open a Ticket', value: 'Select one of the support options below. A private channel will be created and routed to the correct team.', inline: false },
            ...panels.map(panel => ({ name: panel.name, value: truncate(panel.description || 'A private ticket for this department.', 1000), inline: true }))
          )
          .setTimestamp()
        ...
        for (let i = 0; i < panels.length; i += 5) {
          ...
          for (const panel of panels.slice(i, i + 5)) {
      ...
      const panel = {
        ...
        color: parseHexColor(interaction.options.getString('embed-color'), COLORS.primary),
        ticketCounter: 0, messageId: null,
      };
      const channel = await interaction.guild.channels.fetch(panel.channelId);
      const openButton = new ButtonBuilder()
        .setCustomId(`panel_open:${panel.id}`)
        .setLabel(panel.buttonLabel.slice(0, 80))
        .setStyle(buttonStyleFromName(panel.buttonStyle));
      if (panel.buttonEmoji) openButton.setEmoji(panel.buttonEmoji);
