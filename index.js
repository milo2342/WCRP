...
async function refreshApprovalMessage(client, request) {
  ...
  if (!message) return;
  const target = `<@${request.userId}> (${request.userId})`;
  const actionLabel = request.action === 'ban' ? 'Global Ban' : request.action === 'kick' ? 'Global Kick' : request.action === 'timeout' ? 'Global Timeout' : request.action === 'unban' ? 'Global Unban' : 'Global Untimeout';
  const status = request.status === 'pending'
    ? `Waiting for ${request.stage === 'ownership' ? 'ownership' : 'staff'} approval`
    : request.status === 'denied'
      ? `Denied by <@${request.resolvedBy}>`
      : `Approved by <@${request.resolvedBy}>`;
  const extra = request.result ? `\n**Result:** ${request.result.succeeded} server(s) processed, ${request.result.failed} failed.` : '';
  const embed = baseEmbed(`${actionLabel} Request`, [
    `**Target:** ${target}`,
    `**Reason:** ${truncate(request.reason, 700)}`,
    `**Requested by:** <@${request.requestedBy}>`,
    `**Status:** ${status}${extra}`,
  ].join('\n'), request.status === 'denied' ? COLORS.danger : request.status === 'pending' ? COLORS.warning : COLORS.success);
...
function commandData() {
  ...
  add(new SlashCommandBuilder().setName('global').setDescription('Submit a cross-server moderation action for approval.').setDefaultMemberPermissions(PermissionFlagsBits.ManageGuild).setDMPermission(false)
...
function featuresEmbed() {
  return baseEmbed('Bot Command Directory', [
    ...
    '**Global:** `/global ban` `/global kick` `/global timeout` `/unglobal ban` `/unglobal timeout`',
...
async function handleCommand(interaction) {
  ...
  switch (interaction.commandName) {
    ...
    case 'ticket-panel': {
      ...
      if (sub === 'hub') {
        const channel = interaction.options.getChannel('channel', true);
        const ids = String(interaction.options.getString('panel-ids', true)).split(',').map(s => Number(s.trim())).filter(Number.isInteger);
        const panels = ids.map(id => getPanel(id, interaction.guild.id)).filter(Boolean).slice(0, 5);
        if (!panels.length) return interaction.reply({ content: 'No valid panel IDs found.', ephemeral: true });
        const rows = panels.map(panel => new ActionRowBuilder().addComponents(new ButtonBuilder().setCustomId(`panel_open:${panel.id}`).setLabel((panel.buttonLabel || panel.name).slice(0, 80)).setStyle(ButtonStyle.Primary)));
        await channel.send({ embeds: [baseEmbed(interaction.options.getString('title') ?? 'Support Center', interaction.options.getString('description') ?? 'Choose the support department you need.')], components: rows });
      ...
      const panel = {
        ...
        buttonLabel: interaction.options.getString('button-label') ?? 'Open Ticket', ticketCounter: 0, messageId: null,
      };
      const channel = await interaction.guild.channels.fetch(panel.channelId);
      const msg = await channel.send({ embeds: [baseEmbed(panel.title, `${panel.description}\n\nUse the button below to open a private ticket.`)], components: [new ActionRowBuilder().addComponents(new ButtonBuilder().setCustomId(`panel_open:${panel.id}`).setLabel(panel.buttonLabel.slice(0, 80)).setStyle(ButtonStyle.Primary))] });
    ...
    case 'ticket': {
      ...
      if (sub === 'delete') {
        ...
        await interaction.reply({ embeds: [baseEmbed('Deleting Ticket', 'This ticket will be deleted shortly.', COLORS.danger)] });
      ...
      if (sub === 'close' || sub === 'reopen') {
        ...
        if (sub === 'close') {
          ...
          saveTickets();
          return interaction.reply({ embeds: [baseEmbed('Ticket Closed', `Closed by ${interaction.user}.`, COLORS.warning)], components: [ticketButtons(ticket)] });
        }
...
async function handleButton(interaction) {
  ...
  if (type.startsWith('ticket_')) {
    ...
    if (type === 'ticket_close') {
      ...
      if (ticket.closed) return interaction.reply({ content: 'Ticket already closed.', ephemeral: true }); ticket.closed = true; ticket.closedBy = interaction.user.id; await interaction.channel.permissionOverwrites.edit(ticket.openerId,{SendMessages:false}).catch(()=>null); await createTranscript(interaction.channel,ticket,getPanelForTicket(ticket)); saveTickets();
      return interaction.update({ embeds: [baseEmbed('Ticket Closed', `Closed by ${interaction.user}.`, COLORS.warning)], components: [ticketButtons(ticket)] });
    }
    if (type === 'ticket_reopen') { if (!isStaff(interaction)) return interaction.reply({content:'Only staff can reopen tickets.',ephemeral:true}); ticket.closed=false; ticket.closedBy=null; await interaction.channel.permissionOverwrites.edit(ticket.openerId,{SendMessages:true}).catch(()=>null); saveTickets(); return interaction.update({embeds:[baseEmbed('Ticket Reopened',`Reopened by ${interaction.user}.`,COLORS.success)],components:[ticketButtons(ticket)]}); }
